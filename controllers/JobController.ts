...
export default class JobController {
  ...
  // -------------------------
  // MARK URGENT BY TSO
  // POST /api/v1/jobs/mark-urgent-by-tso
  // -------------------------
  public markUrgentByTso = async (req: Request, res: Response) => {
    const schema = Yup.object({
      tso_no: Yup.string().required("tso_no is required"),
      urgent: Yup.boolean().default(true),
      urgent_due_date: Yup.date().nullable(),
      updated_by: Yup.string().uuid().nullable(),
    });
    ...
    try {
      ...
      const { tso_no, ...updatePayload } = body;
      ...
      if (affectedCount === 0) {
        return res.status(404).json({
          success: false,
          error: `No jobs found with tso_no: ${tso_no}`,
        });
      }

      return res.json({
        success: true,
        message: `${affectedCount} job(s) with tso_no ${tso_no} were marked as urgent.`,
        data: {
          tso_no,
          updated_count: affectedCount,
        },
      ...
      console.error("Mark Urgent by TSO Error:", err);
      if (err instanceof Yup.ValidationError) {
  ...
  // -------------------------
  public approveJob = async (req: Request, res: Response) => {
    const schema = Yup.object({
